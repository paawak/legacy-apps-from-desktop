...
public class MonthRollController implements TableListDB, SemaphoresDB, 
        InvoiceDB, ClientAccountDB, DailyOrderBookDB, 
        ConstantsDB, RTFConstants {
    ...
    public void roll(MonthRollView monthRollView, int year, int month){
        ...
        String where = "WHERE "+InvoiceDB.curmonth+"='"+month+"' AND "+InvoiceDB.curyear+"='"+year+"'";
        Object [][] invNos=null;
        invProps = new Properties();
        try{
            deleteInvRec(year,month);
            invNos = invDB.queryMultipleColumns(new String[]{InvoiceDB.accnum,InvoiceDB.invoicenum}, false,where);
        ...
        if(invNos.length==0 ){
            ...
            int option=JOptionPane.showConfirmDialog(monthRollView,"Records for this month already exists: Do you want to ovverwrite?","Record exists",JOptionPane.YES_NO_OPTION);
    ...
    private void startRoll(MonthRollView monthRollView, int year, int month, boolean recordExists){
        //initialise the output stream to write to the file
        
        Object[][] clientDet = null;
        String where  ="WHERE MONTH("+DailyOrderBookDB.consignmentdate+")='"+month+"' AND YEAR("+DailyOrderBookDB.consignmentdate+")='"+year+"' GROUP BY "+DailyOrderBookDB.accnum;
        ...
        for(int i=0; i<clientDet.length; i++){
            ...
            //check whether record exists
            if(recordExists){
                String invNum = invProps.getProperty(clientDet[i][0].toString());//accnum
                if(invNum==null)
                    invNum=smCntr.getID();
            ...
            invVec.addElement( new DailyOrderBookController().getEmplIDLoggedIn());//empl. id
            ...
            try{
                if(recordExists){
                    if(invDB.updateData(invDB.getColumnNames(), invObj.getInvoiceObjectAsArray(),"WHERE "+InvoiceDB.invoicenum+"='"+invObj.getInvoiceNum()+"'")){
                        String details = "Records rolled successfully for client "+clientDet[i][0]+"\n";
                        monthRollView.txtArDetails.append(details);
                    } else{
                        String details = "Records could not be rolled for client "+clientDet[i][0]+"\n";
                        monthRollView.txtArDetails.append(details);
                    }
                } else{
                    if(invDB.insertData(invDB.getColumnNames(), invObj.getInvoiceObjectAsArray())){
    ...
    public void writeInvoiceDetailsToFile(InvoiceObject invObj){
        ...
        //get formatted date
        String invdate = dt.getFormattedDate(invObj.getInvoiceDate().toString(), pattern);
        ...
        String where  ="WHERE MONTH("+DailyOrderBookDB.consignmentdate+")='"+curmonth+"' AND YEAR("+DailyOrderBookDB.consignmentdate+")='"+curyear+"' AND "+DailyOrderBookDB.accnum+"='"+accnum+"' AND "+DailyOrderBookDB.iscredit+"='1' ORDER BY "+DailyOrderBookDB.consignmentdate;
        Object[][] obj = null;
        ...
        DefaultMutableTreeNode mainNode = new DefaultMutableTreeNode(new TagElement(DOCUMENT_HEADER, "\n}\n"));
        //{\fonttbl\f0\fnil Times New Roman;}
        DefaultMutableTreeNode fontTable = new DefaultMutableTreeNode(new TagElement(FONT_TBL, "}\n"));
        //\f0\fnil Times New Roman;
        DefaultMutableTreeNode f0 = new DefaultMutableTreeNode(new TagElement( FONT + FONT_SIZE + FNIL + " " + FONT_FAMILY, ";"));
        fontTable.add(f0);
        mainNode.add(fontTable);
        //{\colortbl\red0\green0\blue0;\red255\green200\blue0;}
        DefaultMutableTreeNode colorTable = new DefaultMutableTreeNode(new TagElement(COLOR_TBL, "}\n"));
        //\red0\green0\blue0;
        DefaultMutableTreeNode color1 = new DefaultMutableTreeNode(new TagElement(RED + "0" + GREEN + "0" + BLUE + "0", ";"));
        colorTable.add(color1);
        //\red255\green255\blue255;
        DefaultMutableTreeNode color2 = new DefaultMutableTreeNode(new TagElement(RED + "255" + GREEN + "255" + BLUE + "255", ";"));
        colorTable.add(color2);
        mainNode.add(colorTable);  
        
        //{\header \qr{\f0\fs24 Invoice No.: XXXX}\par}
        DefaultMutableTreeNode header = new DefaultMutableTreeNode(new TagElement(HEADER, "}\n"));
        ...
        DefaultMutableTreeNode footer = new DefaultMutableTreeNode(new TagElement(FOOTER, "}\n"));
        footer.add(getCentred(COMPANY_NAME + ", " + COMPANY_ADDRESS + ", " + COMPANY_CITY));
        lineCount --;
        //page numbers
        ...
        String[] cellAlignment = {ALIGN_CENTER, ALIGN_LEFT, ALIGN_CENTER, ALIGN_LEFT, ALIGN_RIGHT, ALIGN_RIGHT};
        ...
        for(int i=0;i<obj.length;i++){         
            ...
            //consignment date
            cells.addElement(dt.getFormattedDate(obj[i][1].toString(), pattern));
            //destination
        ...
        mainNode.add(getCentred(DOTTED_LINE));        
        //tax
        mainNode.add(getRighted("(+) Service Tax @ "+moneyFormat.format(taxPercent)+"%: "+moneyFormat.format(tax)));
        mainNode.add(getCentred(DOTTED_LINE));
        //tds
        //grnd total: net recievable amt
        mainNode.add(getAligned("Grand Total: "+netRecievable, ALIGN_RIGHT, BOLD));
        mainNode.add(getCentred(DOTTED_LINE));
        mainNode.add(getAligned("Rupees "+DigitsToFigure.toFigures(""+netRecievable)+" only.", ALIGN_RIGHT, ITALIC));
    ...
    private void printTableRow(DefaultMutableTreeNode parent, Vector cells, int[] cellSizes, String[] cellAlignment){
        ...
        DefaultMutableTreeNode row = new DefaultMutableTreeNode(new TagElement(TABLE_ROW_START, TABLE_ROW_END + "\n"));
        ...
        for(int i=0; i<cells.size(); i++){
            ...
            if(i == cells.size()-1)
                endTag = "\n";
            DefaultMutableTreeNode cellSize = new DefaultMutableTreeNode(new TagElement(TABLE_CELL_X + cellSizes[i] , endTag));
            row.add(cellSize);
        }
                
        for(int i=0; i<cells.size(); i++){
            //\pard\plain \intbl\qc\i\b {\f1\fs24 XXXXXXX}\cell
            DefaultMutableTreeNode cell = new DefaultMutableTreeNode(new TagElement(TABLE_CELL_START + cellAlignment[i] + "{" + FONT + FONT_SIZE + FONT_SIZE_POINTS*2 + " " + cells.elementAt(i) + "}", TABLE_CELL_END + "\n"));
            row.add(cell);
        }
        parent.add(row);
     ...
     */
    private void printPageBreak(DefaultMutableTreeNode parent, String pageTotalAmount){
    ...
    private void setConstants(){
        ...
        if(readMainProps.getVal("IncludeDateTimeWithInvoiceFileName").equals("true"))
            fileName+="_"+dt.getFormattedDate(new Date(),"dd-MM-yyyy_HH-mm-ss");
        fileName+=".rtf";
    }    
    ...
    private void deleteInvRec(int year, int month){
        try{
            String sql ="DELETE FROM "+invoice+" WHERE "+InvoiceDB.curmonth+"='"+month+"' AND "+InvoiceDB.curyear+"='"+year+"'";
     ...
     */
    private void printInvoiceHeader(DefaultMutableTreeNode parent, Object invnum, Object invdate,Object fromDate,Object toDate,ClientAccountObject clObj){
     ...
     */
    private DefaultMutableTreeNode getAligned(String text, String align, String style){
        lineCount ++;
        //\par \pard\plain \qc {\f0\fs24 XXXXXXXXXXX}
        return new DefaultMutableTreeNode(new TagElement(PARD_PLAIN + " " + align + style + "{" + FONT + FONT_SIZE + FONT_SIZE_POINTS*2 + " " + text, "}" + PARA + "\n"));
    }
     ...
     *holds the headers
     */
    private final  String[] headers = {"SL. ","C.NOTE No. ","DATE ","DEST'N ","WT(Kg.gms)","AMOUNT"};
    ...
    /**
     * keeps the count of lines in a page. 
     * this is reset in every new page
     */
    private int lineCount = 0;
    /**
     * keeps track of the total amount for a given page: 
     * should be reset after every page, as well as new file
     */
    ...
    /**
     *This contains(all units are in twips):
     *x of 1st cell, width of 1st cell, width of 2nd cell and so on....
     *Usage:
     *2nd cell loc = cellPosData[0] + cellPosData[1]
     *3rd cell loc = 2nd cell loc + cellPosData[2]
     *this is done in the constructor
     */
    private final int[] cellPosData = {600, 1728, 1728, 1728, 1728, 1728};
    private final static String FONT_FAMILY = "Times New Roman";
    /**
     * this is in points. so this has to be doubled when passed to rtf 
     * as rtf uses double-point format. the page breaks are calculated on this basis.
     */
