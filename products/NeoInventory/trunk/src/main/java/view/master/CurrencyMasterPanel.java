...
public class CurrencyMasterPanel extends javax.swing.JPanel {

    /** Creates new form CurrencyMaster201 */
    public CurrencyMasterPanel() {
        initComponents();
    }
    ...
    private void initComponents() {//GEN-BEGIN:initComponents
        ...
        txtBaseCur = new javax.swing.JTextField();
        ...
        lblFormHeadingH.setText("Currency Master");
        ...
        pnlHome.add(lblFormHeadingH, new org.netbeans.lib.awtextra.AbsoluteConstraints(245, 20, 210, -1));
        ...
        pnlAddMod.add(jLabel2, new org.netbeans.lib.awtextra.AbsoluteConstraints(130, 100, -1, 20));
        ...
        pnlAddMod.add(jLabel3, new org.netbeans.lib.awtextra.AbsoluteConstraints(130, 140, -1, 20));
        ...
        txtBaseCur.setBackground(new java.awt.Color(204, 255, 255));
        txtBaseCur.setHorizontalAlignment(javax.swing.JTextField.RIGHT);
        txtBaseCur.addFocusListener(new java.awt.event.FocusAdapter() {
            public void focusGained(java.awt.event.FocusEvent evt) {
                txtBaseCurFocusGained(evt);
            }
            public void focusLost(java.awt.event.FocusEvent evt) {
                txtBaseCurFocusLost(evt);
            }
        });

        txtBaseCur.addKeyListener(new java.awt.event.KeyAdapter() {
            public void keyReleased(java.awt.event.KeyEvent evt) {
                txtBaseCurKeyReleased(evt);
            }
        });

        pnlAddMod.add(txtBaseCur, new org.netbeans.lib.awtextra.AbsoluteConstraints(280, 180, 230, -1));

        lblFormHeadingH1.setText("Currency Master");
        ...
        pnlAddMod.add(lblFormHeadingH1, new org.netbeans.lib.awtextra.AbsoluteConstraints(245, 20, 210, -1));

        jLabel41.setText("Against Base Currency");
        pnlAddMod.add(jLabel41, new org.netbeans.lib.awtextra.AbsoluteConstraints(130, 180, 130, 20));

        add(pnlAddMod, new org.netbeans.lib.awtextra.AbsoluteConstraints(0, 0, 750, 750));

    }//GEN-END:initComponents

    private void txtBaseCurFocusLost(java.awt.event.FocusEvent evt) {//GEN-FIRST:event_txtBaseCurFocusLost
digitsAfterDecimal=0;
dotTyped=false;

    }//GEN-LAST:event_txtBaseCurFocusLost

    private void txtBaseCurFocusGained(java.awt.event.FocusEvent evt) {//GEN-FIRST:event_txtBaseCurFocusGained
  txtBaseCur.selectAll();
  
    }//GEN-LAST:event_txtBaseCurFocusGained

    private void txtBaseCurKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtBaseCurKeyReleased
StringBuffer sb = new StringBuffer(35);
sb.append(txtBaseCur.getText());
if(sb.length()>30){
    sb.setLength(30);
    txtBaseCur.setText(""+sb);
}
    }//GEN-LAST:event_txtBaseCurKeyReleased

    private void formWindowOpened(java.awt.event.WindowEvent evt) {//GEN-FIRST:event_formWindowOpened
     try{
            accessDB();
            String ss = "SELECT * FROM SemaPhores WHERE TableName = 'CurrencyMaster'";
            
            ResultSet res = stat.executeQuery(ss);
            
            if(!res.next()){
                String ss1="Insert into SemaPhores (TableName,CtrName,MaxCtr) values('"+tableName+"','CRM_',1)";
...
btActivate.setEnabled(true);
    ...
    */
    ...
    private javax.swing.JTextField txtBaseCur;
...
rdbtAllDataSelected();
   ...
   for(i=0;i<rowShown;i++)
       tableShown[i][2] = new Float(""+tableShown[i][2]);
...
ss2="UPDATE "+tableName+ " SET "+ tableColHeads[2]+"  = '"+baseCur+"'  WHERE "+tableColHeads[0]+" = '"+(String)(tableShown[selrow][0])+"' ";
...
txtName.selectAll();
txtBaseCur.setText(""+tableShown[selrow][2]);
txtBaseCur.selectAll();
pnlHome.setVisible(false);   
pnlAddMod.setVisible(true);
click=false;
...
private void addSavePressed(){
        ...
        try{
            baseCur=Float.parseFloat( txtBaseCur.getText() );    
            String s = adjustDecimal(txtBaseCur.getText().trim());
            txtBaseCur.setText(s);
            baseCur=Float.parseFloat(s);
            
        }//end try
        catch(Exception e){
            JOptionPane.showMessageDialog(this,"                   Enter a valid number.                              ","Error: Invalid data in Against Base Currency field.",JOptionPane.ERROR_MESSAGE);
                      txtBaseCur.setText("0");
          txtBaseCur.requestFocus();
...
if(name.equals("")){
         JOptionPane.showMessageDialog(this," The Name-field is empty  ","Error!",JOptionPane.ERROR_MESSAGE);
            ...
            JOptionPane.showMessageDialog(this," Against Base-Currency cannot be zero.  ","Error!",JOptionPane.ERROR_MESSAGE);
          txtBaseCur.setText("0");
          txtBaseCur.requestFocus();
        }//end else if
        else if(baseCur<0){
JOptionPane.showMessageDialog(this,"                         Enter a positive number.                       ","Error: Invalid data in Against Base Currency field.",JOptionPane.ERROR_MESSAGE);                    
txtBaseCur.setText("");
txtBaseCur.requestFocus();
        }
...
private void updateCode() {
    try{
        accessDB();
        ctr++;
        String ss1 = "UPDATE SemaPhores SET  MaxCtr = "+ctr+" WHERE TableName = 'CurrencyMaster' ";
        stat.executeUpdate(ss1);
...
private String adjustDecimal(String f) {
    ...
    System.out.println("3947 : float before adjusting = "+sb);
    int k;
    ...
    String s="";
    s+=sb.toString()+s1;
    System.out.println("3947 : float after adjusting = "+s);
    return s;
}
...
String tableColHeads[],code=new String(),name=new String(),sorted[],unsorted[],key="";
int rowTotal=0,cols=0,i=0,j=0,selrow=0,rowY=0,rowShown=0,h=0,ctr=0,listClicked2=0,digitsAfterDecimal=0;
boolean eleFound=false,tab=false,click=false,duplicate=false,listSelectionChanged=false,dotTyped=false;
StringBuffer sb;
Vector list;
Statement stat;
Connection con;
float baseCur = 0;
String tableName="CurrencyMaster",dbParam[],ss,ss1="",ss2="";    

String tableColHeadsShown[]={"Code","Name","Against Base Cur.","Active"};    
